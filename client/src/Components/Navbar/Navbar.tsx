...
const Navbar = () => {
  ...
  const [isNewItemOpen, setIsNewItemOpen] = useState(false); // 🔹 új state a modalhoz
    ...
    <>
      <nav className="w-full h-[93px] szellit-navbar shadow-md flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4">
          {/* Sell Button – csak bejelentkezett usernek */}
          {user && (
            <button
              ...
              onClick={() => setIsNewItemOpen(true)} // 🔹 modal megnyitása
            >
      ...
      {/* Új hirdetés modal – csak ha meg van nyitva */}
      {isNewItemOpen && (
        <NewItemModal
          onClose={() => setIsNewItemOpen(false)}
          onSuccess={() => {
            setIsNewItemOpen(false);
            // opcionális: pl. frissítés / navigáció
            // goTo("/items");
          }}
        />
      )}
