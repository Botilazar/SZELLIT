...
const ProfilePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const resHonors = await fetch(`http://localhost:5000/api/honors/${userId}`, {
                    headers: { Authorization: token ? `Bearer ${token}` : "" },
                });
                ...
                if (alive) {
                    ...
                    if (isOwner) {
                        const nextBadgeRes = await fetch(`http://localhost:5000/api/badges/next/${userId}`, {
                            headers: { Authorization: token ? `Bearer ${token}` : "" },
                        });
                        ...
                        if (nextBadgeData) {
                            setHonorProgress({
                                ...
                                percent: (honorsData.totalHonors / nextBadgeData.min_honors) * 100,
                            });
                        } else {
    ...
    const handleHonorClick = async () => {
        ...
        try {
            ...
            const badgeRes = await fetch(`http://localhost:5000/api/honors/${userId}`, {
                headers: { Authorization: token ? `Bearer ${token}` : "" },
            });
        ...
        <div className={`max-w-7xl mx-auto p-8 space-y-10 ${isDarkMode ? "text-gray-200" : "text-gray-900"}`}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between szellit-navbar p-6 rounded-2xl shadow-md gap-6">
                        ...
                        {isOwner && honorProgress && (
                            <div className="mt-2 w-56">
                                ...
                                <p className="text-xs text-gray-600 mt-1">
                                    {honorProgress.current}/{honorProgress.needed} honors to next badge
                                </p>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex flex-col items-center gap-3">
                    ...
                    {/* Max Rank */}
                    {isOwner && !honorProgress && (
                        <span className="text-xs text-gray-500 mt-1 font-semibold">(MAX RANK)</span>
                    )}
