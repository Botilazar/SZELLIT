...
const AdminPanelPage = () => {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "admin" | "stduser">(
    "all"
  );
  ...
  return (
    <div
      className={`max-w-7xl mx-auto p-8 space-y-8 ${isDarkMode ? "text-gray-200" : "text-gray-900"}`}
    >
      <h1 className="text-3xl font-bold mb-6 szellit-text">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between szellit-navbar p-4 rounded-2xl shadow-md">
        ...
        {tableType === "users" && (
          <select
            value={roleFilter}
            onChange={(e) =>
              setRoleFilter(e.target.value as "all" | "admin" | "stduser")
            }
            className="szellit-filter-select"
          >
            <option value="all">
              {t("adminPanel.filters.allRoles", "All Roles")}
            </option>
            <option value="admin">
              {t("adminPanel.filters.admins", "Admins")}
            </option>
            <option value="stduser">
              {t("adminPanel.filters.stdUsers", "Standard Users")}
            </option>
          </select>
        )}
      </div>
              ...
              {tableType === "users" ? (
                <>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.id", "ID")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.neptun", "Neptun")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.name", "Name")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.email", "Email")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.role", "Role")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.verified", "Verified")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.created", "Created")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.actions", "Actions")}
                  </th>
                </>
              ) : (
                <>
                  ...
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.title", "Title")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.description", "Description")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.price", "Price")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.category", "Category")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.seller", "Seller")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.city", "City")}
                  </th>
                  <th className="px-6 py-3 font-semibold">
                    {t("adminPanel.tableHeaders.created", "Created")}
                  </th>
            ...
            {tableType === "users" && filteredUsers.length > 0 ? (
                ...
                >
                  ...
                  >
                    <button
                      onClick={() =>
                        alert(
                          t("adminPanel.banAlert", `Ban user ${u.email} (TODO)`)
                        )
                      }
              ...
              filteredItems.map((i, idx) => (
                ...
                >
                  ...
                  <td
                    className="px-6 py-3 truncate max-w-xs"
                    title={i.description}
                  >
                    {i.description}
                  </td>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {tableType === "users" ? (
          <div className="szellit-navbar p-6 rounded-2xl shadow-md">
            <h2 className="text-xl font-semibold mb-3">
              {t("adminPanel.stats.userStats", "User Statistics")}
            </h2>
            <p>
              {t("adminPanel.stats.totalUsers", "Total Users")}: {users.length}
            </p>
            <p>
              {t("adminPanel.stats.admins", "Admins")}:{" "}
              {users.filter((u) => u.role === "ADMIN").length}
            </p>
            <p>
              {t("adminPanel.stats.stdUsers", "Standard Users")}:{" "}
              {users.filter((u) => u.role === "STDUSER").length}
            </p>
          </div>
        ) : (
          <div className="szellit-navbar p-6 rounded-2xl shadow-md">
            <h2 className="text-xl font-semibold mb-3">
              {t("adminPanel.stats.itemStats", "Item Statistics")}
            </h2>
            <p>
              {t("adminPanel.stats.totalItems", "Total Items")}: {items.length}
            </p>
            <p>
              {t("adminPanel.stats.categories", "Categories")}:{" "}
              {Array.from(new Set(items.map((i) => i.category_name))).length}
            </p>
            <p>
              {t("adminPanel.stats.avgPrice", "Average Price")}:{" "}
              {items.length > 0
                ? (
        ...
        <div className="szellit-navbar p-6 rounded-2xl shadow-md">
          <h2 className="text-xl font-semibold mb-3">
            {t("adminPanel.notes.title", "Admin Notes")}
          </h2>
          <p className="szellit-text text-sm">
            {t(
              "adminPanel.notes.description",
